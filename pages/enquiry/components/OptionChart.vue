...
export default {
  ...
  methods: {
    ...
    drawBar() {
      ...
      // 2. 确定数据横纵坐标范围, 边界即原点
      const minX = Math.floor(this.data[0][0]) > dataPoint[0] ? dataPoint[0] : Math.floor(this.data[0][0])
      const maxX = Math.ceil(this.data[length - 1][0]) < dataPoint[0] ? dataPoint[0] : Math.ceil(this.data[length - 1][0])
      const minY = Math.floor(this.data[0][1]) > dataPoint[1] ? dataPoint[1] : Math.floor(this.data[0][1])
      const maxY = Math.ceil(this.data[length - 1][1]) < dataPoint[1] ? dataPoint[1] : Math.ceil(this.data[length - 1][1])
