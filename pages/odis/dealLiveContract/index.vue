...
export default {
  ...
  data() {
    return {
      ...
      // 数据：标的代码
      underlyingData: [], underlyingKey: '', underlyingUrl: '', underlyingParams: {},
  ...
  methods: {
    ...
    async pageDataInit() {
      ...
      pageInitData(this.underlyingUrl, this.underlyingParams).then(res => {
        debugger
        _this.underlyingData = res.content
      }).catch(err => {
        console.log(err)
      })
    },
    beforeInit() {
      const sort = 'id,desc'
      this.url = 'api/deal'
      this.params = { page: this.page, size: 5, sort: sort, ifExist: true }
